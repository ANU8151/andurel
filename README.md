...
- **YAML-Driven Development** - Describe your entire application (models, controllers, jobs, mailers) in a single YAML file and build it in seconds
...
Andurel Blueprint lets you prototype entire features using a single YAML file. It's the fastest way to build your application's foundation.

```bash
# 1. Initialize a draft.yaml file with examples
andurel blueprint init

# 2. Define your models, controllers, jobs, and mailables in draft.yaml

# 3. Build everything in one go
andurel blueprint build

# 4. Want to start over? Erase everything generated by the blueprint
andurel blueprint erase
```
...
models:
  Post:
    title: string:100 validate:required
...
### Blueprint
YAML-driven resource generation.

```bash
andurel blueprint init              # Create a draft.yaml file
andurel blueprint build             # Generate resources from draft.yaml
andurel blueprint erase             # Remove files generated by the blueprint
```
