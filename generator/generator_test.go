	...
	tests := []struct {
	...
	}{
		{
			...
			fn: func() error {
				...
				return gen.GenerateModel("", "", false, nil)
			},
		},
		{
			name: "GenerateController",
			fn: func() error {
				return gen.GenerateController("", "", false, nil)
			},
		},
		{
			name: "GenerateControllerFromModel",
			fn: func() error {
				return gen.GenerateControllerFromModel("", false, nil)
			},
		},
		{
			name: "GenerateView",
...
func TestGenerator_DelegationToCoordinator(t *testing.T) {
	...
	t.Run("GenerateModel delegation", func(t *testing.T) {
		genErr := gen.GenerateModel("", "", false, nil)
		coordErr := gen.coordinator.ModelManager.GenerateModel("", "", false, nil)
	...
	t.Run("GenerateController delegation", func(t *testing.T) {
		genErr := gen.GenerateController("", "", false, nil)
		coordErr := gen.coordinator.GenerateController("", "", false, nil)
...
func TestGenerator_GenerateModelWithTableOverride(t *testing.T) {
	...
	t.Run("validates empty resource name", func(t *testing.T) {
		err := gen.GenerateModel("", "", false, nil)
		if err == nil {
	...
	t.Run("accepts empty table override", func(t *testing.T) {
		err := gen.GenerateModel("User", "", false, nil)
		if err == nil {
	...
	t.Run("validates invalid table override", func(t *testing.T) {
		err := gen.GenerateModel("User", "InvalidTableName", false, nil)
		if err == nil {
